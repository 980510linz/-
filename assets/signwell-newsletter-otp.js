/* SIGN WELL Newsletter OTP · v23.9.82
...
function ensureOverlay(){
  ...
  el.innerHTML=`<section class="sw-otp-card" role="dialog" aria-modal="true" aria-labelledby="swOtpTitle">
    ...
    <div class="sw-otp-stage" data-stage="code">
      ...
      <p class="sw-otp-note">驗證碼 5 分鐘有效。為防止濫用，同一 Email 與同一裝置每天各最多寄送 5 次。</p>
    </div>
...
function start(options){
  ...
  const el=ensureOverlay(),input=el.querySelector('.sw-otp-code-input'),digits=el.querySelector('.sw-otp-code-wrap'),verifyBtn=el.querySelector('.sw-otp-verify'),resendBtn=el.querySelector('.sw-otp-resend'),statusEl=el.querySelector('.sw-otp-status'),expiryEl=el.querySelector('[data-expiry]'),attemptsEl=el.querySelector('[data-attempts]'),emailEl=el.querySelector('[data-email]'),codeStage=el.querySelector('[data-stage="code"]'),successStage=el.querySelector('[data-stage="success"]'),challengeBox=el.querySelector('.sw-otp-challenge');
  emailEl.textContent=maskEmail(email);codeStage.hidden=false;successStage.hidden=true;challengeBox.hidden=true;statusEl.textContent='';statusEl.className='sw-otp-status';input.value='';renderDigits(input,digits);show(el);
  let resolveDone,rejectDone;const done=new Promise((r,j)=>{resolveDone=r;rejectDone=j});
  const flow={el,email,deviceId:getDeviceId(),requestId:'',expiresAt:0,cooldownUntil:0,attemptsRemaining:6,challengeToken:'',turnstileId:null,timer:0,closed:false,status(msg,type){statusEl.textContent=msg||'';statusEl.className='sw-otp-status'+(type?' '+type:'')},close(silent){if(flow.closed)return;flow.closed=true;clearInterval(flow.timer);hide(el);if(activeFlow===flow)activeFlow=null;rejectDone(new Error('已取消 Email 驗證'))}};
  activeFlow=flow;
  ...
  async function requestCode(isResend){
    ...
    try{
      ...
      if(r&&r.status==='challenge_required'){
        ...
        await mountChallenge(flow);flow.status('請完成「我不是電腦」驗證後再重新寄送。','warn');return r;
      }
  ...
  async function verify(){
    ...
    try{
      ...
      if(r&&r.status==='verified'){
        ...
        resolveDone(r);setTimeout(()=>{hide(el);flow.closed=true;if(activeFlow===flow)activeFlow=null},1800);return
      }
...
getDeviceId();
window.SignwellNewsletterOtp={start,getDeviceId,version:'23.9.82'};
})();
